                    ...
                    <table class="table">
                        ...
                        @foreach($product->OrderDetail as $order)
                            <tr class="@if($i%2 != 0) table-success @else text-warning @endif">
                                ...
                                <td>
                                    @if(($order->commission=="Due"))
                                    <button class="btn btn-success" wire:click="Paid({{$order->id}})">Paid</button>
                                    @else
                                    <button class="btn btn-danger" wire:click="Unpaid({{$order->id}})">Unpaid</button>
                                    @endif
                                </td>
                            </tr>
                        @endforeach
